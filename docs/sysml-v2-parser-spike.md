...
Status: spike complete. Companion crate: `crates/sysml-v2-parser-spike`
(throwaway investigation binary + integration tests, not production code).
...
Filing this as a follow-up bug against `holon-viz`, not fixing it in this
spike (out of scope here; this crate is a read-only investigation).
...
- **Closing the actual round-trip requires fixing `holon-viz`'s
  `SysmlV2Emitter` first** — it does not currently emit syntactically valid
  SysML v2 (the `block def` / SysML-v1-terminology bug, plus the
  comment-swallows-brace bug). That's a prerequisite piece of work, tracked
  separately, not something this parser crate can paper over.
- Recommended next steps: (1) file the two `holon-viz` emitter bugs found
  here, (2) fix them and re-run this spike's `holon_viz_emitter_output_does_not_parse_yet`
  test — it's written to fail loudly (not silently pass) once that's done, so
  flipping it is the concrete "did we close the round-trip" signal, (3) pin
  `sysml-v2-parser` to an exact version (not a caret range) given its release
  cadence, and (4) budget for upstream issue triage/patches if we lean on
  constructs beyond basic package/part/import.
...
- `crates/sysml-v2-parser-spike/tests/roundtrip.rs` — 3 pinned regression
  tests (`cargo test -p sysml-v2-parser-spike`), including one that fails
  loudly once the `holon-viz` emitter bugs are fixed (see above).
