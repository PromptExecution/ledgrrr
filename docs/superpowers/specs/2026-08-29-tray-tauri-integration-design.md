...
Status: implemented — original branch (`feat/windows-desktop-dogfood`, PR #213) was superseded by `feat/tray-client-server-port` (PR #216) after `main` independently introduced a client/server settings architecture; this doc is ported here for reference, retrieved verbatim from the superseded branch.
Branch: `feat/windows-desktop-dogfood` (superseded — see above)
