...
wsl2-pwsh-run-window:
    powershell.exe -NoProfile -Command '$env:PATH = "C:\Users\wendy\.cargo\bin;C:\msys64\mingw64\bin;" + $env:PATH; Set-Location "D:\Projects\l3dg3rr"; cargo build -p ledgerr-host --bin host-tauri | Out-Null; Start-Process -FilePath "D:\Projects\l3dg3rr\target\debug\host-tauri.exe" -WorkingDirectory "D:\Projects\l3dg3rr"'
...
wsl2-pwsh-run-window-phi4:
    powershell.exe -NoProfile -Command '$env:PATH = "C:\Users\wendy\.cargo\bin;C:\msys64\mingw64\bin;" + $env:PATH; Set-Location "D:\Projects\l3dg3rr"; cargo build -p ledgerr-host --bin host-tauri --features mistralrs-llm | Out-Null; Start-Process -FilePath "D:\Projects\l3dg3rr\target\debug\host-tauri.exe" -WorkingDirectory "D:\Projects\l3dg3rr"'

# Build the mdBook playbook assets, then launch the Tauri desktop host whose
...
# Build the docs and launch the Tauri desktop host. In the window, explicitly
# select "Windows AI / Foundry Local" to use the Foundry Local OpenAI-compatible endpoint.
...
#   ui/docs/public/viz-manifest.json — generated by cargo run -p xtask-mcpb -- export-viz-manifest
#
...
check-drift:
    ...
    echo ""
    echo "=== check-drift: viz-manifest.json ==="
    cargo run -p xtask-mcpb -- export-viz-manifest 2>&1
    if ! git diff --exit-code ui/docs/public/viz-manifest.json; then
        echo "FAIL: viz-manifest.json is out of date — run: cargo run -p xtask-mcpb -- export-viz-manifest"
        FAIL=1
    else
        echo "PASS: viz-manifest.json is up to date"
    fi
...
# One-time local setup: installs kani-verifier and its pinned nightly toolchain
# (separate from the workspace's stable toolchain — see kani-toolchain.toml).
# Only needs to run once per machine; `cargo kani` reuses the installed toolchain.
kani-setup:
    cargo install --locked kani-verifier
    cargo kani setup

# Run the kani-proofs harness suite locally. Requires `just kani-setup` first.
# Gives feedback in seconds instead of the 5-6h+ round trip through CI's
# "Kani Proofs" workflow — use this to iterate on a proof before pushing.
# If a harness hangs, Ctrl-C and try `cargo kani --harness <name> --solver z3
# --output-format old` from kani-proofs/ to see the raw CBMC verdict (the
# structured `cargo kani` output can crash-on-parse for non-default solvers).
kani-check:
...
# that a `git push` command exited 0. Compares local HEAD (or a given branch) to
# its remote counterpart and fails loudly if they've diverged or the local branch
# is ahead. Defaults to the current branch and same-named remote branch; pass
# both args to check a rebased local branch against a differently-named PR branch,
# e.g. `just verify-pushed rebase/165-foo feat/118-foo`.
verify-pushed local=`git branch --show-current` remote=`git branch --show-current`:
    #!/bin/bash
    set -euo pipefail
    git fetch origin "{{remote}}" --quiet
    LOCAL=$(git rev-parse "{{local}}")
    REMOTE=$(git rev-parse FETCH_HEAD)
    if [ "$LOCAL" = "$REMOTE" ]; then
        echo "PASS: {{local}} ($LOCAL) == origin/{{remote}}"
    else
        echo "FAIL: {{local}} ($LOCAL) != origin/{{remote}} ($REMOTE)"
        if git merge-base --is-ancestor "$REMOTE" "$LOCAL" 2>/dev/null; then
            echo "  origin/{{remote}} is behind — push: git push origin {{local}}:{{remote}}"
        elif git merge-base --is-ancestor "$LOCAL" "$REMOTE" 2>/dev/null; then
            echo "  local {{local}} is behind origin/{{remote}} — pull/rebase first"
        else
            echo "  histories have diverged — a force-push may be required; review before pushing"
        fi
        exit 1
    fi
