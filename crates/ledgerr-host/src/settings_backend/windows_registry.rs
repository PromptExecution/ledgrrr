...
/// Registry path under HKCU where the app's settings are stored.
const SETTINGS_PATH: &str = r"software\b00t\settings";

/// A settings backend backed by the Windows Registry.
///
/// Always opens the single fixed [`SETTINGS_PATH`] key. Tests that need
/// isolation from the real registry should construct a `SettingsStore` over
/// an explicit [`crate::settings_backend::JsonFileBackend`] via
/// [`crate::settings::SettingsStore::with_backend`] instead of going through
/// this backend at all — see that constructor's doc comment.
///
...
impl WindowsRegistryBackend {
    ...
    ///
    /// `path` is accepted for API-compatibility with [`super::create_backend`]
    /// (and as a JSON-file fallback path if this constructor fails) but is
    /// otherwise unused: the registry always uses the single fixed
    /// [`SETTINGS_PATH`] key.
    pub fn new(_path: &Path) -> Result<Self, SettingsBackendError> {
        let backend = Self;
