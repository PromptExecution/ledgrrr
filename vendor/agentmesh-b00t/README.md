...
> This is a vendored copy of `agentmesh` 4.0.0 (crates.io) with two changes:
> `serde` and `thiserror`'s dependency requirements are loosened from
> upstream's exact pins (`=1.0.228`, `=2.0.18`) to compatible ranges
> (`>=1.0.228, <2` / `>=2.0.18, <3`), so they can coexist with anything
> else in a consuming workspace's Cargo.lock that needs a newer patch
> version (here: `scxml`, which needs `serde >=1.0.229` /
> `thiserror >=2.0.19` — see `_b00t_#1177` P7). Everything else, including
