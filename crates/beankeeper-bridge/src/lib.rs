...
impl From<EntryError> for BridgeError {
    fn from(e: EntryError) -> Self {
        BridgeError::EntryConstruction(e.to_string())
    }
}
impl From<beankeeper::types::AccountCodeError> for BridgeError {
    fn from(e: beankeeper::types::AccountCodeError) -> Self {
        BridgeError::AccountCode(e.to_string())
    }
}
...
impl BridgeResult {
    ...
    pub fn asset_entry(&self) -> &Entry {
        &self.asset_entry
    }
    pub fn offset_entry(&self) -> &Entry {
        &self.offset_entry
    }
}
...
pub fn convert(
    ...
    let asset_account = Account::new(
        AccountCode::new(&config.asset_code)?,
        &config.asset_name,
        AccountType::Asset,
    );
    let offset_account = Account::new(
        AccountCode::new(&config.offset_code)?,
        &config.offset_name,
        config.offset_kind.clone().into(),
    );

    // OFX positive = money in → Debit asset / Credit offset.
    let (asset_entry, offset_entry) = if ofx_decimal.is_sign_positive() {
        (
            Entry::debit(asset_account, money)?,
            Entry::credit(offset_account, money)?,
        )
    } else {
        (
            Entry::credit(asset_account, money)?,
            Entry::debit(offset_account, money)?,
        )
    };
...
mod tests {
    ...
    fn cfg_expense() -> ConversionConfig {
        ConversionConfig {
            asset_code: "1000".into(),
            asset_name: "Checking".into(),
            offset_code: "6000".into(),
            offset_name: "Rent Expense".into(),
    ...
    fn different_fit_id_produces_different_id() {
        ...
        assert_ne!(
            ra.id, rb.id,
            "different fit_id must produce different blake3 ID"
        );
    }
