...
fn doc_01_mcp_boundary_tool_catalog_exposes_reduced_top_level_surface() {
    ...
    // rather than a hardcoded literal, so adding a published tool cannot
    // silently drift this count out of sync — see contract.rs's own
    // module doc: "Rust code is the only source of truth for the published
    // MCP surface... Drift... should fail tests."
    assert_eq!(tools.len(), ledgerr_mcp::contract::PUBLISHED_TOOLS.len());
    ...
    assert!(tools.contains(&"ledgerr_gcp_billing".to_string()));
