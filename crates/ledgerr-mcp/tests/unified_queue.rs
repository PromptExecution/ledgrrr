...
use ledgerr_mcp::{
    ClassifyTransactionRequest, FetchQueueRequest, IngestStatementRowsRequest, QueueItemType,
    QueueStatus, TurboLedgerService, TurboLedgerTools,
};
...
fn test_fetch_queue_returns_all_types() {
    ...
    // Fetch work queue
    let response = svc
        .fetch_work_queue(FetchQueueRequest {
            item_types: None,
            statuses: None,
            updated_after: None,
            limit: 100,
    ...
    let item_types: HashSet<QueueItemType> =
        response.items.iter().map(|item| item.item_type).collect();

    assert!(!response.items.is_empty(), "Queue should not be empty");
    assert!(
        item_types.contains(&QueueItemType::Flag),
        "Should contain Flag items"
    );
}
...
fn test_fetch_queue_updated_after_filter() {
    ...
    assert_eq!(
        response.items.len(),
        0,
        "Should return no items for future date"
    );
}
...
fn test_fetch_queue_sorts_by_created_at_desc() {
    ...
    let timestamps: Vec<&String> = response.items.iter().map(|item| &item.created_at).collect();
...
fn test_fetch_queue_pagination_works() {
    ...
    assert_eq!(
        page1.total_count, page2.total_count,
    ...
    assert_eq!(
        page1_ids.intersection(&page2_ids).count(),
        0,
...
fn test_fetch_queue_manual_change_tx_document_ref() {
    ...
    let manual_changes: Vec<_> = response
        .items
        .iter()
    ...
    if !manual_changes.is_empty() {
        ...
        for item in manual_changes {
            assert_eq!(
                item.item_type,
                QueueItemType::ManualChange,
                "Item type should be ManualChange"
            );
            assert_eq!(
                item.status,
                QueueStatus::Resolved,
                "Manual changes should be Resolved"
            );
