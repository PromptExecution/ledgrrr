...
//!
//! Platform dispatch:
//! - **Windows**: Uses the native `Shell_NotifyIconW` tray via the shared
//!   `ledgerr_host::tray::runtime::run()` — the same tested toggle/
//!   notification/toast menu the (now-retired) standalone `host-tray.exe`
//!   used, with "Show Window" redirected to this app's own webview instead
...
pub fn setup_tray(app: &tauri::App) {
    ...
    std::thread::spawn(move || {
        ...
        let store = SettingsClient::new();
        let result = ledgerr_host::tray::runtime::run(store, move || {
            if let Some(window) = show_app_handle.get_webview_window("main") {
                // unminimize() before show(): Tauri's show() maps to Win32
                // SW_SHOW, which does not clear a window's iconic (minimized)
                // bit — only SW_RESTORE (unminimize()) does.
                window
                    .unminimize()
